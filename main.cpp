...
public:
    ...
    void makeMove(int r,int c,char sym) { grid[r][c] = sym; }
    void undoMove(int r,int c) { grid[r][c] = ' '; }
    ...
    bool checkWin(char sym) {
        for(int i=0;i<size;i++){
            ...
            for(int j=0;j<size;j++){
                if(grid[i][j]!=sym) rowWin=false;
                if(grid[j][i]!=sym) colWin=false;
            }
        ...
        for(int i=0;i<size;i++){
            ...
            if(grid[i][size-i-1]!=sym) diag2=false;
        }
        return diag1||diag2;
    }
...
public:
    ...
    void getBestMove(Board &board,int &row,int &col){
        char ai=symbol, human=(symbol=='X')?'O':'X';
        int bestScore=-10000;
        for(int i=0;i<board.getSize();i++){
            for(int j=0;j<board.getSize();j++){
...
private:
    Board board;
    Player *player1, *player2, *current;
public:
    ...
    void start(){
        ...
        if(mode==1){
            string n1,n2;
            cout << "Enter Player1 name: "; cin >> n1;
            cout << "Enter Player2 name: "; cin >> n2;
            player1=new Player(n1,'X'); player2=new Player(n2,'O');
        }
        else{
            string n;
            int diff;
            cout << "Enter Your Name: "; cin>>n;
            cout << "Select Difficulty (1-Easy,2-Hard): "; cin>>diff;
            player1=new Player(n,'X');
            player2=new AIPlayer("Computer",'O',(diff==1)?Easy:Hard);
        }
        current=player1;
